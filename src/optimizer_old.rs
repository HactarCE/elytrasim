...
impl State {
    ...
    fn state_goodness(&self) -> Goodness {
        ...
        //     // punish being low, reward distance.
        const TARGET_Y: f64 = 10.0;
        (if self.pos.y < TARGET_Y {
            self.pos.y - TARGET_Y
        } else {
            (self.pos.y - TARGET_Y) / 12.0
        } + self.pos.z / 100.0) as _
...
impl Pitches {
    ...
    fn jittered(&self) -> Self {
        const JITTER: f32 = 0.1;
        let mut rng = rand::rng();
        let mut pitches = self.clone();
        for pitch in pitches.0.iter_mut() {
            *pitch += rng.random_range(-JITTER..JITTER);
            clamp_pitch(pitch);
        }
        pitches
    }
    ...
    fn gradient_descent_step(&mut self, init_vel: Vel3, learning_rate: f64) {
        let mut jittered = self.jittered();
        let grads = jittered.grad(init_vel).collect::<Vec<_>>();
